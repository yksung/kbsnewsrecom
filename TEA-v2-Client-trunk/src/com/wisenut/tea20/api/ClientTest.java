...
public class ClientTest {
	...
	public static final int TEA_PORT = 11890;
	...
	public static void main(String[] args) {
		...
		// test_getSimilarDoc(); test_getSimilarDoc2(); test_getSimilarDoc3();
    ...
    public static void test_getSimilarDocWithContent()
    {
    	...
    	//get similar document list by model  		
    	String documentFields = "DIST_ID|TITLE|CONTENT";
		List<Pair<Double>> similarDocumentList = new ArrayList<Pair<Double>>();
		List<Pair<String>> similarDocumentContentList = new ArrayList<Pair<String>>();
  	   	ArrayList<String> docIdListForFiltering =  new ArrayList<String>();
    	String prefix = ""; // none
    	
		getSimilarDocWithContent( content, documentFields, similarDocumentList, similarDocumentContentList, docIdListForFiltering, prefix); 
    	...
    	for (int i = 0; i < similarDocumentContentList.size(); i++) {
 			Pair<String> item = similarDocumentContentList.get(i);
 			if (null == item) {
 				continue;
 			}
 			System.out.println( item.key() + "^" + item.value() );
    ...
    public static List<Pair<Double>> getSimilarDoc( String content ) {
    	...
    	ArrayList<String> docListForFiltering = new ArrayList<String>();
    	String prefix = "" ;
    ...
    public static List<Pair<Double>> getSimilarDocWithContent( String content, String documentFields, List<Pair<Double>> similarDocumentList, List<Pair<String>> similarDocumentContentList, ArrayList<String> docListForFiltering, String prefix ) {
   	 
    	teaClient = new TeaClient(TEA_IP, TEA_PORT);
    	String query = "CONTENT_PLAIN" + "$!$" + content;
         
        return teaClient.getSimilarDocWithContent( "kbs", query, documentFields, "10", similarDocumentList, similarDocumentContentList, docListForFiltering, prefix);
    }
           
    public static List<Pair<Integer>> extractNer( String content, ArrayList<String> docListForFiltering, String prefix ) {
      	 
    	teaClient = new TeaClient(TEA_IP, TEA_PORT);
    	String query = "CONTENT_PLAIN" + "$!$" + content;
       
        return teaClient.extractNerForPlainText("kbs", query,"10", docListForFiltering, prefix );
    }
	...
	public ArrayList<String> search(String query, int listNo, int isDebug) {
		...
		for (String col : collectionArr) {
			...
			if (debug)
				System.out.println(" - ranking : basic, rpf, 10000");
			ret = search.w3SetRanking(col, "basic", "rpf", 10000);

			if (debug)
				System.out.println(" - highlight : 1,1");
			...
			if (debug)
				System.out.println(" - query analyzer : 1,1,1,1");
			...
			if (debug)
				System.out.println(" - document fields : " + documentFields);
			ret = search.w3SetDocumentField(col, documentFields);

			if (debug)
				System.out.println(" - page info : " + pageNum + ", " + listNo);
			ret = search.w3SetPageInfo(col, pageNum, listNo);
		}

		if (debug)
			System.out.println(" - search ip : " + SEARCH_IP);
		if (debug)
			System.out.println(" - search port : " + SEARCH_PORT);
		if (debug)
			System.out.println(" - search timeout : " + SEARCH_TIMEOUT);
