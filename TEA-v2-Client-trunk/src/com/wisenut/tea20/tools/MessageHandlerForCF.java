...
public class MessageHandlerForCF {
	...
	public MessageHandlerForCF(String ip, int port) {
		this.ip = ip;
		this.port = port;
	}

	public MessageHandlerForCF(String ip, int port, int interval, int maxRetry) {
		this.ip = ip;
		this.port = port;
	...
	public MessageHandlerForCF(String ip, int port, int initInterval, int interval, int maxRetry, int waitTimeout) {
		this.ip = ip;
		this.port = port;
	...
	public SocketMessageForCF getResponse(SocketMessageForCF request) {
		SocketMessageForCF toReturn = new SocketMessageForCF(true);
	...
	private String receive() throws Exception {
		try {
			byte [] bRead = new byte [MAX_BUFFER_SIZE];
			int numRead = 0;
			/*
			byte [] message = null;
			...
			*/
			...
			do {
				...
				message += new String(bRead, MESSAGE_ENCODING).trim();
